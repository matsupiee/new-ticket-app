...
const TicketTypeUpdateMutation = graphql(`
  mutation TicketTypeUpdate($input: TicketTypeUpdateInput!) {
    ticketTypeUpdate(input: $input) {
      ticketType {
        id
        name
        description
...
export function EditTicketTypeDialog({
  ...
  const [ticketTypeUpdateResult, updateTicketType] = useMutation(
    TicketTypeUpdateMutation,
  );
  ...
  const form = useForm<TicketTypeFormData>({
    defaultValues: {
      ...
      basePrice: String(ticketType.basePrice),
      capacity: String(ticketType.capacity),
      maxNumPerApply: String(ticketType.maxNumPerApply),
  ...
  const onSubmit = async (data: TicketTypeFormData) => {
    ...
    if (result.error) {
      console.error('Error updating ticket type:', result.error);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>チケットタイプを編集</DialogTitle>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <FormField
              ...
              rules={{
                required: '枠数は必須です',
                min: { value: 1, message: '枠数は1以上である必要があります' },
              }}
              render={({ field }) => (
                <FormItem>
            ...
            <FormField
              ...
              rules={{
                ...
                min: {
                  value: 1,
                  message: '1申込あたりの最大枚数は1以上である必要があります',
                },
              }}
              render={({ field }) => (
                <FormItem>
